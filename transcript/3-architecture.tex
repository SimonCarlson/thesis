...
Finally Section~\ref{sec:use-cases-examples-topologies} shows the architecture applied to
different use cases.
...
their responsibilities, and their intended functionality.
Section~\ref{ssec:what-is-a-device} briefly covers devices,
Section~\ref{ssec:what-is-a-server} covers the topic of servers, and
...
\begin{itemize}
    \item A server is enrolled and has a valid certificate
    \item A server is included in a device's list of servers
    \item A server can request authorization tokens and be authorized to update a device
    \item An operator can reach the server and is authorized by the server to query device
            status and upload manifests and images
\end{itemize}
...
Like a list of servers introduced in Section~\ref{ssec:what-is-a-device}, devices also
...
interact with devices but other operators must interact with devices through servers. Yet
...
versions they are using. This information is held by the server in form of device
profiles. An operator can query the server selecting the devices of interest based on
vendor or class ID or possibly firmware version. 
...
server when deciding about which devices to update, and should be given help filling in
the details of the manifest to ensure its correctness.
...
\begin{itemize}
    ...
    \item An operator can request authorization tokens and be authorized to update a
            device
    \item An operator is trusted by a server to query device status and send manifests and
...
updates might communicate with servers or operators outside the contrained network. If the
same key pair is used for device-to-device communication and updates and an attacker gets
ahold of that key they can use it both inside and outside the constrained network. Using
the same key pair for device-to-device communication and updates makes the consequences of
losing keys greater. 
...
can be used on the same device, an implementor should consider the granularity of the key
...
authorize themselves, i.e. a device, operator, or server. The access token would have to
be of a lightweight variant since they are to be used on constrained devices, but which
variant is implementation specific. Tokens will be sent with signed manifests and images
to devices and the resource sought for is the ability to update the device. The device
will thus not respond with a particular resource as in Figure~\ref{fig:ace-flow}, but
instead if the token is valid recognize the originator of the update as an authorized one
...
Who is supposed to authorize? Devices will only allow communication from trusted sources
specified in predetermined whitelists of servers and operators. These trusted servers and
operators must be authorized to access the resources of the device (in this case, the
update mechanism). This access control is granted through authorization tokens. Frameworks
such as OAuth and ACE use proof-of-possession tokens that are bound to a cryptographic
key. When receiving a proof-of-possession token, a receiver must verify the key bound to
the token is held by the client sending the token. This means that if an operator sends an
update with a token to a server which forwards the update and token to a device, the
device must be able to verify with the operator that it holds the key bound to the token.
The device will in essence not be able to verify this as it received the message from the
server, and not the operator.

In order to remedy this, the operator's authorization token will be used to authorize the
communication with the server and then terminate. If the token is valid, the server itself
will ask for a new token which is used for transporting the update to the device. The
device can then verify the token's key with the server which should be the holder of that
key. But what about operators sending manifests directly to devices and devices pulling
updates from the server? There are four scenarios where tokens must be requested from each
of the three parties:

\begin{itemize}
    \item When an operator sends a signed image (and possibly manifest) to a server
    \item When an operator send a signed manifest to a device
    \item When a server sends a signed image (and possibly manifest) to a device
    \item When a device pulls an update from a server or wants to register
\end{itemize}

It is clear that all parties must be able to reach the authorization server. A device
could also have a secondary reason for doing so which is introspection requests when
verfifying the token. If the verification process is complex and the device very
constrained, additional information can be requested from the authorization server making
the verification process easier for the device. This would however incur extra
communication which is a significant energy sink in constrained devices. The trade off
between verifying locally and on the authorization server must be kept in mind when
implementing an update handler on a device.
...
devices are enrolled and registered, and re-enrollment and updating
...
token. The server accepts the update and stores it for future use. When the device is
about to pull the update, it first requests an authorization token. The
...
lightbulb controller in a smart home. The technician requests a token and sends it
...
to devices in its network. Both a lightbulb controller, the intended target, and a
thermostat controller receives the update. The thermostat controller is supposed to only
accept updates from the thermostat vendor, thus the lightbulb vendor technician is
unauthorized to perform this update. The thermostat controller rejects the update while
the lightbulb controller accepts it and updates itself.

\begin{figure}
    \caption{A lightbulb vendor technician updates a lightbulb controller in a smart home,
                but the thermostat controller rejects it due to authorization issues.}
