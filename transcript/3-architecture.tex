...
differentiate between authors of updates and operators, this architecture will for
simplicity regard them as the same actor. In the same way, the architecture does not
differentiate between device and network operators. An operator is an actor that tracks
device statuses and prepares, signs, and sends updates to devices. The figure gives a
brief introduction to the concepts needed to understand the architecture, these concepts
will be explained in more detail throughout sections
\ref{sec:roles}-\ref{sec:authorization}. In this particular example the update procedure
is initiated by the operator by querying the server for device status and then pushing an
update. The manifest and image are distributed together. The steps can be briefly
explained as:
...
\begin{enumerate}
    ...
    \item The signed manifest and image are sent to the server with the authorization
            token.
    \item The server requests an authorization token from the authorization server.
    ...
    \item The device verifies the manifest and image and applies the update.
...
Section~\ref{ssec:use-cases-examples-topologies} shows the architecture applied to
different use cases.
