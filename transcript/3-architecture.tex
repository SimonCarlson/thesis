...
can verify server certificates. A server is thus any machine that is enrolled, has a
...
the server, verifies it using the already received manifest, and updates. Both the pull
...
will thus not respond with a particular resource but instead if the token is valid
...
whitelists of servers and operators, this tells devices which sources of communication to
allow. This does not mean however that each of the trusted sources are allowed to apply
any update they so wish, they need to provide tokens to prove authorization for such
resources.
...
operator sends an update with a token to a server which forwards the update and token to a
device, the device must be able to verify with the \textit{operator} that it holds the key
bound to the token and not the server. The device will in essence not be able to verify
this as it received the message from the server, and not the operator.
...
key. What about when operators send manifests directly to devices and when devices pull
updates from the server? There are four scenarios where tokens must be requested from the
different parties:
...
token. Authorization enables operators, servers, and devices to send and receive updates
and enables use cases such as differential updates. If a device is running several
