...
The architecture of the update mechanism shall enable the lifecycle described in the
...
not needed, but as that approach is less fit for scaling IoT networks asymmetric
encryption will be encounted for. If using symmetric encryption, everything is the same
but the certificate authority can be ignored.

Figure \ref{fig:architecture} shows the proposed architecture. 

\begin{figure}
    \caption{The proposed architecture.}
    \label{fig:architecture}
    \begin{tikzpicture}[box/.style={rectangle,draw,minimum width=3cm, minimum height=2cm},
        line/.style={->,>=latex}]
        ...
        \draw[line] (device.south) -- node[above,sloped] 
            {0.0 Enroll} (CA.east); 
        \draw[line] ([yshift=0.8cm]device.west) -- node[above] {0.1 Register} ([yshift=0.8cm]server.east);
        \draw[line] ([yshift=0.8cm]operator.east) -- node[above] {0.3 Manifest and image} ([yshift=0.8cm]server.west);
        \draw[line] (server.east) -- node[above] {0.4 Manifest and image} (device.west);
        ...
        \draw[line] (server.east) -- node[above] {1.5 Image} (device.west);
