...
% TODO: Implementation needs an update handler that 1) can decrypt the manifest 2) can
% parse the decrypted manifest 3) can decrypt the image 4) can verify the decrypted
% manifest 5) can prepare the decrypted image for boot
% Mention that how to store manifest and image is further discussed later
...
% TODO: Implementation needs an endpoint which devices can POST their credentials to that
% creates a profile. Also some API for querying devices, where an operator can ask for
% devices of a vendor, of a class of a vendor, of all devices etc. API for devices to poll
% and get updates.
