...
is a 16-bit integer used to detect duplicate message and to match Acknowledgment or Resets
with their initiating requests.

Following the header is the zero to eight bit Token value, which in turn is follow by zero
or more Options. One of the options offered is the observe option, which allows a client
to be notified by the server when a particular resource changes. Lastly comes the optional
