...
perform secure updates. %As a rule of thumb, each byte sent expends as
...
optionally included using the options field of the manifest. The structure of the options
field is shown in \ref{fig:option-struct}. The included options must be sorted by their
option code in ascending order to calculate a delta between the current and preceeding
option. This is the way CoAP implements options, and it allows for a smaller amount of
bits to encode the option code when the codes get larger. The options and their codes are
presented in Table \ref{tab:option-codes}.
