...
an update procedure. It consists of an update server and a client. The prototype is using
a pull model where the client initiates the update procedure. The notion of an operator is
cut out as the prototype will not be used in a real deployment, as such there is only the
...
% TODO: Port not yet done, have no hardware

% TODO: Mention COSE here and assumptions made to make it work
The client makes use of Contiki-NGs blocking CoAP API. CoAP request
...
manifest or not, offloading any decision making from the device. The prototype only makes
use of one manifest thus such logic is not implemented, but in real deployments the
\texttt{update/manifest} resource would handle that by using the information from the
devices' profile.
...
receives these chunks and reassembles the manifest. % TODO: Show callbacks in Contiki
After the manifest in its entirety has been transferred, the client decrypts and parses
the manifest and checks its validity by comparing vendor and class IDs. As described in
